...
generate_one_plot<-function(type_of_plot="MSE",kappa1=1){
  ...
  if(type_of_plot=="MSE"){
    empir_index=1:3
    plot_data_set$theory_limit=ConjectureSolution[,1]^2+kappa1^2*(ConjectureSolution[,3]-1)^2
  }else{
    empir_index=4:6
    plot_data_set$theory_limit=kappa1^2*ConjectureSolution[,3]/(kappa1*sqrt(kappa1^2*ConjectureSolution[,3]^2+ConjectureSolution[,1]^2))
  }
