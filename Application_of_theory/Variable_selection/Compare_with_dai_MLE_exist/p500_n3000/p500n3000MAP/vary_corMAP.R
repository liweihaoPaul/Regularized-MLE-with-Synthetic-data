...
for(rho in c(0,0.2,0.4,0.6,0.8)){
  repeat_function<-function(rep_i){
    ...
    # flatten_matrix <- as.vector(sigma_hat)
    # 
    # # Calculate the 80th percentile of the absolute values
    # percentile_80 <- quantile(abs(flatten_matrix), 0.8)
    # 
    # # Set entries below the 80th percentile to zero
    # sigma_hat[abs(sigma_hat) < percentile_80] <- 0
  ...
  save(data_save_list, file = paste0('p500n3000MAPvary_cor_',rho,'.RData'))
}
