...
// [[Rcpp::export]]
double estimate_eta_square_cpp(const Eigen::MatrixXd & X, 
  ...
  double mean_val = betahat_minus_i_T_Xi_seq.mean();

  double result = (betahat_minus_i_T_Xi_seq.array() * betahat_minus_i_T_Xi_seq.array()).mean() - mean_val * mean_val;
